...
DIAGNOSTIC_ONLY_FUNC30_MODES = {550, 560, 570, 580}
DIAGNOSTIC_FORMULA_ALLOWLIST = {"chan-debug.txt"}
...
EXPECTED_FORMULA_SNIPPETS = {
  ...
  "chan-first-buy-dynamics.txt": [
    ...
    "APS:=TDXDLL1(30,H,L,430);",
    "APE:=TDXDLL1(30,H,L,440);",
    "CPS:=TDXDLL1(30,H,L,450);",
    "CPE:=TDXDLL1(30,H,L,460);",
    ...
    "ACOK:=APS>0 AND APE>APS AND CPS>APE AND CPE>CPS;",
    "BARSLAST(BSP=1 AND NEX AND DOK AND WSP AND WVP AND WMA AND ACOK AND MAR>0 AND MAR<100 AND SPR>0 AND SPR<100 AND VPR>0 AND VPR<100)<10;",
  ],
  ...
  "chan-first-sell-dynamics.txt": [
    ...
    "DOK:=MOD(INTPART(DVG/16),2)=1;",
    "ACOK:=APS>0 AND APE>APS AND CPS>APE AND CPE>CPS;",
    "BARSLAST(BSP=11 AND NEX AND DOK AND WSP AND WVP AND WMA AND ACOK AND MAR>0 AND MAR<100 AND SPR>0 AND SPR<100 AND VPR>0 AND VPR<100)<10;",
  ],
...
def validate_diagnostic_formula_boundaries(formula_texts):
  errors = []
  for name in sorted(formula_texts):
    text = formula_texts[name]
    if name in DIAGNOSTIC_FORMULA_ALLOWLIST:
      continue
    active_text = strip_tdx_comments(text)
    modes = {int(item) for item in FUNC30_REF.findall(active_text)}
    diagnostics = sorted(modes & DIAGNOSTIC_ONLY_FUNC30_MODES)
    if diagnostics:
      errors.append(f"{name}: diagnostic Func30 modes are debug-only: {diagnostics}")
  return errors
...
def self_test() -> int:
  ...
  if main_errors != ["chan-main.txt: main formula should stay lightweight, unexpected Func30 modes: [320]"]:
    ...
    return 1
  diagnostic_errors = validate_diagnostic_formula_boundaries({
    "chan-debug.txt": "FLT:=TDXDLL1(30,H,L,550);\nNDS:=TDXDLL1(30,H,L,560);\n",
    "chan-first-buy.txt": "BSP:=TDXDLL1(30,H,L,40);\n",
  })
  if diagnostic_errors:
    print(f"self-test failed: diagnostic formula allowlist {diagnostic_errors!r}", file=sys.stderr)
    return 1
  diagnostic_errors = validate_diagnostic_formula_boundaries({
    "chan-main.txt": "FLT:=TDXDLL1(30,H,L,550);\n",
    "chan-first-buy.txt": "NDS:=TDXDLL1(30,H,L,560);\nNDR:=TDXDLL1(30,H,L,580);\n",
  })
  expected_diagnostic_errors = [
    "chan-first-buy.txt: diagnostic Func30 modes are debug-only: [560, 580]",
    "chan-main.txt: diagnostic Func30 modes are debug-only: [550]",
  ]
  if diagnostic_errors != expected_diagnostic_errors:
    print(f"self-test failed: diagnostic formula boundaries {diagnostic_errors!r}", file=sys.stderr)
    return 1
...
def main() -> int:
  ...
  diagnostic_formula_errors = validate_diagnostic_formula_boundaries(formula_texts)
  ...
  expected_debug_comments = [
    ...
    "区间套低级别背驰段起点端点编号：一基",
    "区间套低级别背驰段终点端点编号：一基",
    "区间套递归背驰语义：1趋势/2盘整/3小转大",
    "区间套递归确认位图：1内部/2背驰/4创新/8小转大",
    "区间套低级别背驰方向：买1/卖-1",
  ...
  expected_debug_lines = [
    ...
    "DRAWNUMBER(BSP<>0 AND DGS>0,L*0.972,DGS)",
    ...
    "DRAWNUMBER(BSP<>0 AND APS>0,H*1.060,APS)",
    ...
    "DRAWNUMBER(NDS>0,L*0.925,NDS)",
    "DRAWNUMBER(NDS>0 AND NDF>0,H*1.080,NDF)",
    "DRAWNUMBER(NDS>0,L*0.920,NDR)",
  ...
  if (missing or empty or undocumented or invalid_modes or stale_comments or
    ...
    for item in diagnostic_formula_errors:
      print(f"diagnostic formula error: {item}", file=sys.stderr)
